...
def inline_handler(query):
    ...
    if media_count == 0:
        download_event = None
        is_primary_downloader = False

        with download_lock:
            if platform_id in active_downloads:
                # A download of the same post is in progress
                download_event = active_downloads[platform_id]
            else:
                download_event = threading.Event()
                active_downloads[platform_id] = download_event
                is_primary_downloader = True

        if not is_primary_downloader:
            ...
            try:
                ...
                with download_lock:
                    download_event.set()
                    if platform_id in active_downloads:
                        del active_downloads[platform_id]
